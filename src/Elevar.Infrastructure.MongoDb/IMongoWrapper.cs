...
namespace Elevar.Infrastructure.MongoDb
{
    public interface IMongoWrapper
    {
        ...
        List<TDocument> FindPaging<TDocument>(string collectionName, FilterDefinition<TDocument> filter, out long totalRecords, int pageSize = 100, int currentPage = 1);

        Task<(List<TDocument>, long)> FindPagingAsync<TDocument>(string collectionName, FilterDefinition<TDocument> filter, int pageSize = 100, int currentPage = 1);
